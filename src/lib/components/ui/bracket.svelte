			...
			<div class="bracket-matches">
				{#each ms as match, mi}
					<div class="bracket-match" class:played={match.winnerTeamId !== null}>
						...
						{#if round > 1}
							<div
								class="bracket-line-top"
								class:active={match.homeTeamId !== null || match.awayTeamId !== null}
							></div>
						{/if}
						...
						<div class="bg-bg-surface clipped border border-border-subtle p-3 sm:p-4">
							<div class="flex items-center justify-center gap-2 sm:gap-3">
								<div class="min-w-0 flex-1 text-right">
									<p
										class="font-display truncate text-sm font-bold uppercase tracking-wide sm:text-base"
									>
										{match.homeTeam?.name ?? '?'}
									</p>
									{#if match.homeScore !== null}
										<p
											class="scoreboard-font mt-1 text-xl sm:text-2xl {match.winnerTeamId ===
											match.homeTeamId
												? 'text-accent-primary'
												: 'text-text-muted'}"
										>
								...
								<div class="flex-shrink-0 text-center">
									{#if match.status === 'finished' && match.winnerTeam}
										...
										<p class="font-mono text-[10px] text-accent-primary uppercase tracking-wider">
											Winner
										</p>
									...
									{/if}
									<p class="text-text-muted mt-1 font-mono text-[10px] uppercase tracking-wider">
