<script lang="ts">
	...
	let bracketGenerated = $derived(tournament.status !== 'draft');
	...
	async function handleDelete(teamId: string) {
		try {
			...
			pageError = e instanceof Error ? e.message : 'Gagal menghapus tim';
		}
	}
	...
	async function handleGenerateBracket() {
		...
		try {
			...
			pageError = e instanceof Error ? e.message : 'Gagal generate bracket';
		} finally {
			generating = false;
		}
	}
	...
	async function handleStartMatch(matchId: string) {
		try {
			...
			pageError = e instanceof Error ? e.message : 'Gagal mulai pertandingan';
		}
	}
	...
	async function handleScore(match: Match, homeScore: number, awayScore: number) {
		try {
			...
			pageError = e instanceof Error ? e.message : 'Gagal update skor';
		}
	}
</script>
...
<div class="mx-auto min-h-screen max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
	...
	{#if pageError}
		<div class="mt-8">
			<p class="text-accent-live font-mono text-xs">{pageError}</p>
		</div>
	{:else if tournament}
