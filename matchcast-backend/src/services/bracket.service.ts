...
export function shuffleArray<T>(array: T[]): T[] {
	...
	for (let i = arr.length - 1; i > 0; i--) {
		...
		const a = arr[i];
		const b = arr[j];
		/* istanbul ignore next — unreachable, guarantees non-null */
...
function calcPropagateSlots(round1Matches: number): Set<number> {
	const slots = new Set<number>();
	for (let m = 1; m <= round1Matches; m++) {
		const idx = Math.ceil(m / 2) - 1;
		slots.add(idx * 2 + (m % 2 === 1 ? 0 : 1));
	}
	return slots;
}
...
function generateRoundMatches(
	r: number,
	seedTeams: TeamWithSeed[],
	byes: number,
	round1Matches: number,
	prevRoundMatches: number,
	matchOrderStart: number,
): { matches: RoundMatch[]; matchOrderEnd: number } {
	...
	for (let i = 0; i < matchesInRound; i++) {
		...
		if (r === 2 && byes > 0) {
			...
			if (byeIdx < byes && !propagateSlots.has(homeSlot)) {
				const bt = byeTeams[byeIdx];
				if (bt) m.homeTeamId = bt.id;
				byeIdx++;
			}
			if (byeIdx < byes && !propagateSlots.has(awaySlot)) {
				const bt = byeTeams[byeIdx];
				if (bt) m.awayTeamId = bt.id;
				byeIdx++;
			}
		}
...
// ponytail: bracket planning is inherently combinatorial — extracting further adds
// more param-passing than it saves
function planBracketMatches(
	seeds: TeamWithSeed[],
	round1Matches: number,
	byes: number,
	totalRounds: number,
	...
	for (let i = 0; i < round1Matches; i++) {
		matches.push({
			round: 1,
			matchOrder: matchOrder++,
			homeTeamId: seeds[byes + i * 2]?.id ?? null,
			awayTeamId: seeds[byes + i * 2 + 1]?.id ?? null,
			status: "scheduled",
		});
	}
...
export async function generateBracket(tournamentId: string, ownerId: string) {
	try {
		...
		return await prisma.$transaction(async (tx) => {
			...
			await Promise.all(
				teamsWithSeed.map((t) =>
					tx.team.update({ where: { id: t.id }, data: { seed: t.seed } }),
				),
			);
