...
export const publicRoutes: FastifyPluginAsync = async (app) => {
	await app.register(rateLimit, {
		...
		errorResponseBuilder: (_req, context) => {
			let cooldown = "60 detik";
			if (context.after) {
				cooldown = context.after.endsWith("s")
					? context.after.slice(0, -1) + " detik"
					: context.after;
			}
			return {
				success: false,
				error: {
					message: `Too many requests. Try again in ${cooldown}.`,
					code: "RATE_LIMITED",
				},
			};
		},
	...
	app.get(
		...
		wrapHandler(async (request, reply) => {
			...
			const tournament =
				await tournamentService.getPublicTournamentBySlug(slug);
