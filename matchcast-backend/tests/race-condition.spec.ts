...
test.describe("Race conditions", () => {
  test("concurrent generate-bracket only creates one set of matches", async ({ request }) => {
    ...
    const results = await Promise.allSettled([
      request.post(`/tournaments/${t.id}/generate-bracket`, {
        data: {},
    ...
    const detailRes = await request.get(`/tournaments/${t.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  test("concurrent score update only applies once", async ({ request }) => {
    ...
    const genRes = await request.post(`/tournaments/${t.id}/generate-bracket`, {
      data: {},
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    const results = await Promise.allSettled([
      request.patch(`/matches/${matchId}/score`, {
