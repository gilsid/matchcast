...
test.describe('Auth errors', () => {
	...
	test('protected route with empty Bearer returns 401 UNAUTHORIZED or INVALID_TOKEN', async ({
		request
	}) => {
		const res = await request.get('/api/tournaments', {
...
test.describe('API error paths', () => {
	test('DELETE team: happy path', async ({ request }) => {
		...
		const res = await request.delete(`/api/tournaments/${t.id}/teams/${team.id}`, {
			data: {},
	...
	test('DELETE team: non-existent team returns 404 NOT_FOUND', async ({ request }) => {
		...
		const res = await request.delete(`/api/tournaments/${t.id}/teams/nonexistent-id`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test('DELETE team: non-existent tournament returns 404 NOT_FOUND', async ({ request }) => {
		...
		const res = await request.delete(`/api/tournaments/nonexistent-id/teams/nonexistent-id`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test('DELETE team: after bracket generated returns 403 BRACKET_LOCKED', async ({ request }) => {
		...
		const res = await request.delete(`/api/tournaments/${t.id}/teams/${teamId}`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test("DELETE team: other user's tournament returns 404 NOT_FOUND", async ({ request }) => {
		...
		const otherCtx = await apiRequest.newContext({ baseURL: 'http://localhost:5173' });
		const { token: otherToken } = await loginUser(otherCtx);
		const res = await otherCtx.delete(`/api/tournaments/${t.id}/teams/${team.id}`, {
			data: {},
	...
	test('PATCH start: finished match returns 400 INVALID_STATUS', async ({ request }) => {
		...
		await request.patch(`/api/matches/${m.id}/start`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		await request.patch(`/api/matches/${m.id}/score`, {
			data: { homeScore: 3, awayScore: 1 },
			headers: { Authorization: `Bearer ${token}` }
		});

		const res = await request.patch(`/api/matches/${m.id}/start`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test('PATCH start: match with null team slots returns 400 MISSING_TEAMS', async ({ request }) => {
		...
		const futureMatch = matches.find((m) => m.homeTeamId === null || m.awayTeamId === null);
		if (!futureMatch) {
			test.skip();
			return;
		}

		const res = await request.patch(`/api/matches/${futureMatch.id}/start`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test('PATCH start: non-existent match returns 404 NOT_FOUND', async ({ request }) => {
		const { token } = await loginUser(request);
		const res = await request.patch('/api/matches/nonexistent-id/start', {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test("PATCH start: other user's match returns 403 UNAUTHORIZED", async ({ request }) => {
		...
		const otherCtx = await apiRequest.newContext({ baseURL: 'http://localhost:5173' });
		const { token: otherToken } = await loginUser(otherCtx);
		const res = await otherCtx.patch(`/api/matches/${m.id}/start`, {
			data: {},
			headers: { Authorization: `Bearer ${otherToken}` }
		});
		const body = await res.json();
	...
	test('PATCH score: tied score returns 400 TIED_SCORE', async ({ request }) => {
		...
		const res = await request.patch(`/api/matches/${m.id}/score`, {
	...
	test('PATCH score: non-existent match returns 404 NOT_FOUND', async ({ request }) => {
		const { token } = await loginUser(request);
		const res = await request.patch('/api/matches/nonexistent-id/score', {
			data: { homeScore: 3, awayScore: 1 },
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test("PATCH score: other user's match returns 403 UNAUTHORIZED", async ({ request }) => {
		...
		const otherCtx = await apiRequest.newContext({ baseURL: 'http://localhost:5173' });
		const { token: otherToken } = await loginUser(otherCtx);
		const res = await otherCtx.patch(`/api/matches/${m.id}/score`, {
			data: { homeScore: 3, awayScore: 1 },
			headers: { Authorization: `Bearer ${otherToken}` }
		});
		const body = await res.json();
	...
	test('POST generate-bracket: sequential double-generate returns 409 BRACKET_EXISTS', async ({
		request
	}) => {
		const suffix = Date.now().toString(36);
		const { token } = await loginUser(request);
		...
		const res1 = await request.post(`/api/tournaments/${t.id}/generate-bracket`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		expect(res1.status()).toBe(200);

		const res2 = await request.post(`/api/tournaments/${t.id}/generate-bracket`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body2 = await res2.json();
	...
	test('POST generate-bracket: less than 2 teams returns 400 NOT_ENOUGH_TEAMS', async ({
		request
	}) => {
		const suffix = Date.now().toString(36);
		const { token } = await loginUser(request);
		...
		const res = await request.post(`/api/tournaments/${t.id}/generate-bracket`, {
			data: {},
			headers: { Authorization: `Bearer ${token}` }
		});
		const body = await res.json();
	...
	test('GET /t/:slug/matches: happy path', async ({ request }) => {
		...
		const res = await request.get(`/api/t/${t.slug}/matches`);
		const body = await res.json();
		expect(res.status()).toBe(200);
		expect(body.success).toBe(true);
	...
	test('GET /t/:slug/matches: non-existent slug returns 404 NOT_FOUND', async ({ request }) => {
		const res = await request.get('/api/t/nonexistent-slug-12345/matches');
		const body = await res.json();
		expect(res.status()).toBe(404);
		expect(body.success).toBe(false);
		expect(body.error.code).toBe('NOT_FOUND');
	...
	test('GET /api/tournaments/:id: non-existent ID returns 404 NOT_FOUND', async ({ request }) => {
		const { token } = await loginUser(request);
	...
	test("GET /api/tournaments/:id: other user's tournament returns 404 NOT_FOUND", async ({
		request
	}) => {
		const suffix = Date.now().toString(36);
		const { token: ownerToken } = await loginUser(request);
		...
		const otherCtx = await apiRequest.newContext({ baseURL: 'http://localhost:5173' });
		const { token: otherToken } = await loginUser(otherCtx);
		const res = await otherCtx.get(`/api/tournaments/${t.id}`, {
			headers: { Authorization: `Bearer ${otherToken}` }
		});
		const body = await res.json();
		expect(res.status()).toBe(404);
	...
	test('GET /t/:slug: non-existent slug returns 404 NOT_FOUND', async ({ request }) => {
		const res = await request.get('/api/t/nonexistent-slug-12345');
		const body = await res.json();
		expect(res.status()).toBe(404);
		expect(body.success).toBe(false);
		expect(body.error.code).toBe('NOT_FOUND');
	...
	test('DELETE team: other user trying to delete from bracket-locked tournament returns 404', async ({
		request
	}) => {
		const suffix = Date.now().toString(36);
		const { token: ownerToken } = await loginUser(request);
		...
		const otherCtx = await apiRequest.newContext({ baseURL: 'http://localhost:5173' });
		const { token: otherToken } = await loginUser(otherCtx);
		const res = await otherCtx.delete(`/api/tournaments/${t.id}/teams/${teams[0]!.id}`, {
			data: {},
			headers: { Authorization: `Bearer ${otherToken}` }
		});
		expect(res.status()).toBe(404);
